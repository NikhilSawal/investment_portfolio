...
class IndexSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        yield {
            ...
            's&p_500_delta'     : delta_pattern.sub(r'\1', index[1]).replace(",",""),
            's&p_500_delta(%)'  : delta_perc_pattern.sub(r'\1', index[2]),
            'dow_30'            : index[3].replace(",",""),
            'dow_30_delta'      : delta_pattern.sub(r'\1', index[4]).replace(",",""),
            'dow_30_delta(%)'   : delta_perc_pattern.sub(r'\1', index[5]),
            'nasdaq'            : index[6].replace(",",""),
            'nasdaq_delta'      : delta_pattern.sub(r'\1', index[7]).replace(",",""),
